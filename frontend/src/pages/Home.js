...
//         statusMessage += `${steps.selfie_uploaded ? '✅' : '⬜'} Photo Upload\n`;
//         statusMessage += `${steps.biometric_uploaded ? '✅' : '⬜'} Signature/Biometric Upload\n`;
...
//       let resultMessage = '✅ Photo uploaded successfully!\n\n';
...
//               <p>Upload a clear photo for face verification.</p>
//             </div>
...
//           <div className="form-card">
//             <div className="card-header">
//               <h2>👆 Step 4: Signature/Biometric</h2>
//               <p>Upload signature or fingerprint.</p>
//             </div>
//             <form onSubmit={handleBiometricSubmit}>
//               <div className="form-group">
//                 <label>Upload Signature/Biometric Image *</label>
//                 <div className="file-upload-wrapper">
//                   <input
//                     type="file"
//                     accept="image/*"
...
function Home() {
  ...
  const handleEmailLookup = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        statusMessage += `${steps.document_uploaded ? '✅' : '⬜'} Document Verification\n`;
        statusMessage += `${steps.selfie_uploaded ? '✅' : '⬜'} Photo Upload\n`;
        statusMessage += `${steps.biometric_uploaded ? '✅' : '⬜'} Signature/Fingerprint Upload\n`;
        statusMessage += `${steps.face_verified ? '✅' : '⬜'} Face Verification\n\n`;
  ...
  const handlePhotoSubmit = async (e) => {
    ...
    if (!userId || !photoImage) {
      setMessage('❌ Please upload a photo!');
      return;
    }

    setLoading('photoUpload', true);

    const formDataPhoto = new FormData();
    formDataPhoto.append('user_id', userId);
    formDataPhoto.append('selfie_image', photoImage); // Backend still expects 'selfie_image'

    try {
      const response = await axios.post(`${API_URL}/api/images/upload-selfie`, formDataPhoto);
      ...
      if (response.data.duplicate_check) {
        const dupCheck = response.data.duplicate_check;
        if (dupCheck.is_duplicate) {
          resultMessage += `⚠️ WARNING: Duplicate photo detected!\n`;
          resultMessage += `Found ${dupCheck.duplicate_count} similar photo(s) in database.\n\n`;
        } else {
          resultMessage += `✅ No duplicate photos found - Unique face detected.\n\n`;
        }
      }
  ...
  const handleLivePreviewCapture = async () => {
    ...
    try {
      ...
      const liveness = response.data.liveness;
      const docComp = response.data.document_comparison;
      const photoComp = response.data.selfie_comparison; // Backend still calls it selfie_comparison
      const dupDetection = response.data.duplicate_detection;
      ...
      resultMessage += `📷 Photo vs Live:\n`;
      resultMessage += `   ${photoComp.match ? '✅' : '❌'} ${photoComp.match ? 'MATCH' : 'NO MATCH'}\n`;
      resultMessage += `   Similarity: ${photoComp.similarity_percentage.toFixed(2)}%\n\n`;
      ...
      if (dupDetection) {
        ...
        if (dupDetection.is_duplicate) {
          ...
          resultMessage += `• Document Check: ${dupDetection.document_duplicate ? '❌ Duplicate Found' : '✅ Unique'}\n`;
          resultMessage += `• Photo Check: ${dupDetection.selfie_duplicate ? '❌ Duplicate Found' : '✅ Unique'}\n`;
          resultMessage += `• Live Check: ${dupDetection.live_duplicate ? '❌ Duplicate Found' : '✅ Unique'}\n\n`;
      ...
      if (response.data.verification_passed && (!dupDetection || !dupDetection.is_duplicate)) {
        ...
        resultMessage += '✓ Liveness check passed\n';
        resultMessage += '✓ Document face matched\n';
        resultMessage += '✓ Photo face matched\n';
        resultMessage += '✓ No duplicates detected\n\n';
        resultMessage += '🔒 Identity verified! Registration approved.\n';
        resultMessage += '✨ Your account is now permanently verified (no expiry).';
      } else {
        ...
        if (!photoComp.match) {
          failures.push('✗ Photo face does not match');
        }
        if (dupDetection && dupDetection.is_duplicate) {
          failures.push('✗ Duplicate user detected in system');
        }
      ...
      if (error.response?.status === 400) {
        ...
        if (detail?.includes('No face detected')) {
          ...
          errorMessage += 'Please complete previous steps:\n';
          errorMessage += '• Upload document with clear photo\n';
          errorMessage += '• Upload photo image\n';
          errorMessage += '• Then try live capture again';
        } else {
          errorMessage += detail || 'Unknown error occurred';
        }
  ...
  // Step 5: Upload signature/fingerprint
  const handleSignatureSubmit = async (e) => {
    e.preventDefault();

    if (!userId || !signatureImage) {
      setMessage('❌ Please upload signature/fingerprint data!');
      return;
    }

    setLoading('signatureUpload', true);

    const formDataSignature = new FormData();
    formDataSignature.append('user_id', userId);
    formDataSignature.append('biometric_image', signatureImage); // Backend still expects 'biometric_image'

    try {
      await axios.post(`${API_URL}/api/biometric/upload`, formDataSignature);
      setMessage('✅ Signature/Fingerprint uploaded! 🎉 All steps completed!\n\nClick "Check Verification Status" to view results.');
    } catch (error) {
      console.error('Error details:', error.response?.data);
      setMessage('❌ Error: ' + (error.response?.data?.detail || error.message));
    }

    setLoading('signatureUpload', false);
  };
  ...
  return (
    <div className="home-container">
      <div className="form-section">
          ...
          <div className="form-card">
            ...
            <form onSubmit={handlePhotoSubmit}>
              ...
              <button 
                type="submit" 
                className="btn btn-secondary"
                disabled={loadingStates.photoUpload}
              >
                {loadingStates.photoUpload ? (
                  <>
                    <span className="spinner"></span> Uploading...
                  </>
                ) : (
        ...
        {/* Step 4: Signature/Fingerprint Upload */}
        {userId && (
          <div className="form-card">
            <div className="card-header">
              <h2>✍️ Step 4: Signature/Fingerprint</h2>
              <p>Upload your signature or fingerprint for verification.</p>
            </div>
            <form onSubmit={handleSignatureSubmit}>
              <div className="form-group">
                <label>Upload Signature/Fingerprint Image *</label>
                <div className="file-upload-wrapper">
                  <input
                    type="file"
                    accept="image/*"
              ...
              <button 
                type="submit" 
                className="btn btn-success"
                disabled={loadingStates.signatureUpload}
              >
                {loadingStates.signatureUpload ? (
                  <>
                    <span className="spinner"></span> Uploading...
                  </>
                ) : (
            ...
            <div className="resume-status">
              ...
              <div className="progress-list">
                ...
                <div className={`progress-item ${resumedUser.steps_completed.biometric_uploaded ? 'completed' : ''}`}>
                  ...
                  <span>Signature/Fingerprint Upload</span>
                </div>
                <div className={`progress-item ${resumedUser.steps_completed.face_verified ? 'completed' : ''}`}>
                  <span className="progress-icon">
                    {resumedUser.steps_completed.face_verified ? '✅' : '⬜'}
